...
def parse_argument():
    parser = argparse.ArgumentParser(description='some data processing tools')
    parser.add_argument('filename', type=match_file, help='file to processing')
    parser.add_argument('--nor', help='normalized data', action='store_true')
    parser.add_argument('--gaus', type=int, help='gaussian filter 1d data', default=0)
    parser.add_argument('--fold', help='fold and average', action='store_true')
    parser.add_argument('--err', help='error bar of data', action='store_true')
    parser.add_argument('--int_cn', type=float, nargs=2, help='integrate gofr data to coordination number', default=None)
...
def main():
    args = parse_argument()
    for data_file in args.filename:
        ...
        if args.nor:
            ...
            gaus_data = data.copy()
            for i in range(1, data_range):
                gaus_data[:, i] = gaussian_filter1d(data[:, i], args.gaus)
        ...
        elif args.int_cn is not None:
            from scipy import integrate
            rho = 32/(9.86**3)
            x = data[:, 0]
            mask = (x >= args.int_cn[0]) & (x <= args.int_cn[1])
            filtered_x = x[mask]
